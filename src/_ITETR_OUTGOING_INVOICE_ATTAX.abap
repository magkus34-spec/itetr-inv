*&---------------------------------------------------------------------*
*& Report /ITETR/OUTGOING_INVOICE_ATTAX
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT /itetr/outgoing_invoice_attax.

DATA: BEGIN OF ls_oginv,
        ...
        awtyp TYPE awtyp,
      END OF ls_oginv,
      lt_oginv      LIKE TABLE OF ls_oginv,
...
SELECT docui bukrs belnr gjahr awtyp
  FROM /itetr/inv_oginv             "#EC CI_NOFIELD
  INTO TABLE lt_oginv
 WHERE stacd NOT IN ('', '2')
   AND attax EQ space.

LOOP AT lt_oginv INTO ls_oginv.
  REFRESH: lt_objhdr, lt_solix.
  CLEAR: ls_docinfo.

  CALL FUNCTION '/ITETR/INV_OUTINV_DOWNLOAD'
    EXPORTING
      iv_document_uid = ls_oginv-docui
  ...
  CASE ls_oginv-awtyp.
    WHEN 'VBRK'.
      lv_objkey  = ls_oginv-belnr.
      lv_objtype = ls_oginv-awtyp.
    WHEN 'RMRP'.
      CONCATENATE ls_oginv-belnr ls_oginv-gjahr INTO lv_objkey.
      lv_objtype = 'BUS2081'.
    WHEN 'BKPF' OR 'BKPFF' OR 'REACI'.
      CONCATENATE ls_oginv-bukrs ls_oginv-belnr ls_oginv-gjahr INTO lv_objkey.
      lv_objtype = 'BKPF'.
  ENDCASE.
  ...
  UPDATE /itetr/inv_oginv SET attax = 'X' WHERE docui = ls_oginv-docui.
  COMMIT WORK AND WAIT.
ENDLOOP.